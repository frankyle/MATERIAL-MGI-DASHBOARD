...
const TasksTable = () => {
  ...
  const handleViewTask = (id) => navigate(`/task-details-view/${id}`);
  const handleEdit = (id) => navigate(`/task-update-edit/${id}`);
    ...
    <div className="overflow-x-auto shadow-md sm:rounded-lg">
          ...
          {tasks.length === 0 ? (
            ...
            tasks.map((task) => (
              <tr key={task.id} className="bg-white border-b">
                ...
                <td className="px-6 py-4 text-black">{task.description}</td>
                ...
                <td className="px-6 py-4 text-black">{formatDate(task.created_at)}</td>
                <td className="px-6 py-4 text-black">{formatDate(task.updated_at)}</td>
                <td className="px-6 py-4 text-black">{task.priority || 'N/A'}</td>
                <td className="px-6 py-4 text-black flex space-x-4">
      ...
      {imageModalOpen && selectedImage && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center" onClick={closeImageModal}>
          <div className="bg-white p-4 rounded-lg max-w-full max-h-full overflow-auto" onClick={(e) => e.stopPropagation()}>
            <img src={selectedImage} alt="Task" className="max-w-full max-h-screen object-contain" />
          </div>
        </div>
      )}
    </div>
